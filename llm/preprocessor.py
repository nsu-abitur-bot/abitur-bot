...
Вместо таблиц создавай связные текстовые абзацы. Каждая строка таблицы должна быть
самодостаточной: явно указывай объект строки, показатель, тип данных и значение.
Например, вместо "Прикладная математика — 120" пиши:
"Для направления 'Прикладная математика' количество платных мест: 120".
7. Каждый абзац, пункт списка и смысловой блок должен сохранять локальный контекст,
чтобы он был понятен отдельно от соседних блоков после chunking в LightRAG.
Не оставляй голые числа, итоги, сроки, условия или короткие пункты без указания,
к чему они относятся.
8. Если данные относятся к конкретному разделу, явно повторяй этот контекст внутри
соответствующего блока: платные места, бюджетные места, магистратура, бакалавриат,
специалитет, иностранные граждане, прием в НГУ, год приема, направление подготовки
и другие важные ограничения из исходного текста.
9. Не заменяй специфичные сущности общими словами. Пиши "бюджетные места",
"платные места", "места по договорам об оказании платных образовательных услуг",
"срок подачи документов" и т.п., если это следует из исходного контекста.
10. Итоги и агрегированные значения переписывай с полным контекстом.
Например, вместо "Итого — 965" пиши:
"Итого количество платных мест приема в НГУ на 2026 год: 965".
11. Не изменяй фактические данные: числа, даты, названия, условия, формулировки
ограничений. Разрешено только явно переносить контекст из заголовков, разделов и
таблиц внутрь абзацев.
"""
