...
Циклов сейчас нет (их разобрали в #310), поэтому запрет безусловный: любая
взаимная пара — ошибка. Если когда-нибудь понадобится временно разрешить
существующие пары, положи их по одной в строке в tools/layers_baseline.txt:
проверка будет падать только на парах, которых в этом файле нет, а на
исчезнувших — требовать обновить файл, чтобы починенное нельзя было вернуть.
...
def main() -> int:
    ...
    if added:
        ...
        print(
            ...
            "Как разбирали предыдущие — в задаче #310."
        )
        return 1
