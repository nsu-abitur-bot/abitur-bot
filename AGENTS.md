...
- **LightRAG** (`rag/graph_memory.py`) хранит и граф сущностей/связей, и векторный индекс чанков — единым встроенным стором на диске (`data/lightrag/<graph_id>/`, `graph_id="abitur_kb"`). Отдельного ChromaDB нет. Чанкинг внутри LightRAG (`chunk_token_size=400`, overlap 50). Опционально `LIGHTRAG_STORAGE=postgres` переносит KV-сторы и doc_status в PostgreSQL (JSONB-таблицы `lightrag_*`, графи разделены колонкой `workspace`); векторный индекс и граф остаются файловыми, pgvector не нужен (VDB-таблицы создаются заглушками). Существующие JSON-данные переносит `uv run python -m rag.migrate_json_to_pg` (идемпотентно, есть `--dry-run`). Дефолт — прежний файловый режим.
