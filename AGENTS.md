...
- **Справочник факультетов** (`Faculty`/`Program`, `db/postgres/services/faculty.py`) — авторитетный источник для CRAG. Автозаливается при старте из `db/seed/faculties.json`, редактируется через `/api/v1/faculties`.
