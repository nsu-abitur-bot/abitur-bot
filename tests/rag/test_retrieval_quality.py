...
FACTUAL_CASES = [
    pytest.param(
        "Проходной балл на ФИТ бюджет",
        "267",
        "Проходной балл ФИТ = 267",
        marks=pytest.mark.xfail(
            reason="all-MiniLM-L6-v2 не находит числовой факт для ФИТ"
        ),
    ),
    ...
    pytest.param(
        "Проходной балл геология НГУ бюджет",
        "222",
        "Проходной балл ГГФ = 222",
        marks=pytest.mark.xfail(
            reason="all-MiniLM-L6-v2 не находит числовой факт для ГГФ"
        ),
    ),
    pytest.param(
        "Проходной балл факультет естественных наук бюджет",
        "233",
        "Проходной балл ФЕН = 233",
        marks=pytest.mark.xfail(
            reason="all-MiniLM-L6-v2 не находит числовой факт для ФЕН"
        ),
    ),
    pytest.param(
        "Проходной балл история бюджет НГУ",
        "248",
        "Проходной балл ГИ история = 248",
        marks=pytest.mark.xfail(
            reason="all-MiniLM-L6-v2 не находит числовой факт для ГИ"
        ),
    ),
    ("Проходной балл юриспруденция бюджет НГУ", "253", "Проходной балл ИФП юр = 253"),
    pytest.param(
        "Проходной балл психология бюджет НГУ",
        "241",
        "Проходной балл психология = 241",
        marks=pytest.mark.xfail(
            reason="all-MiniLM-L6-v2 не находит числовой факт для психологии"
        ),
    ),
    pytest.param(
        "Стоимость обучения ФИТ",
        "195",
        "Стоимость ФИТ ~195000",
