...
`SECRET_KEY` для тестов задаёт `tests/conftest.py`: без него импорт
`api/auth/security.py` падает (так и задумано для прода), и pytest прерывался бы
на сборе. Если ключ задан в окружении, тесты берут его.
